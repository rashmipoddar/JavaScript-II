...
// Problem 1
// Filter the name of the companies whose donations exceed $150
let highDonationCompaniesArray = runners.filter(runner => {
    return runner.donation > 150;   
});
// console.log(highDonationCompaniesArray);
let highDonationCompanies = highDonationCompaniesArray.map((company => company.company_name));
console.log(highDonationCompanies);

// Problem 2
// Filter the name of companies whose donation is less than $50
let lowDonationCompaniesArray = runners.filter(runner => {
  return runner.donation < 50;   
});
console.log(lowDonationCompaniesArray);
let lowDonationCompanies = lowDonationCompaniesArray.map((company => company.company_name));
console.log(lowDonationCompanies);

// Problem 3
// Return full name and email of each person to send an email saying thank you for attending the event.

let companyRepresentatives = runners.map(runner => `{fullname: ${runner.first_name} ${runner.last_name}, email: ${runner.email}}`)
console.table(companyRepresentatives);